...
class BangaloreWumpusWorld:
    ...
    def find_path_astar(self):
        ...
        # Helper: cost to enter a neighbor cell
        def move_cost(nx, ny):
            cell = self.grid[ny][nx]
            ctype = cell.get('type', 'empty')
            # impassable/avoid: pits and cows
            if ctype == 'pit' or ctype == 'cow':
                return None  # signal impassable
            base = cell.get('weight', 1)
            if ctype == 'traffic_light':
                return base + 5  # traffic light penalty
            # default cost
            return base
        ...
        while open_heap:
            ...
            if current == goal:
                ...
                return path  # excludes start, suitable for execute_path()
